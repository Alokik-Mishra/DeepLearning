...
class RMSpropOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        ###################################################
        # Update params and cache                         #
        ###################################################
        for k in grads:
            cache[k] = gamma*cache[k] + (1-gamma)*(grads[k]**2)
            params[k] -= np.divide(learning_rate*grads[k],  (np.sqrt(cache[k] + eps)))
        self.cache = cache
...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        params, grads = model.params, model.grads
        ###################################################
        # Update t, momentums, velocitys and params       #
        ###################################################
        t += 1.0
        for k in grads:
            ...
            #Correcting for bias
            momentums_int = np.divide(momentums[k],(1 - beta1**t))
            velocitys_int = np.divide(velocitys[k],(1 - beta2**t))
            
            params[k] -= np.divide(learning_rate, (np.sqrt(velocitys_int + eps))) * momentums_int
...
class NadamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            velocitys[k] = (beta2 * velocitys[k]) + ((1 - beta2) * (grads[k]**2))
            #Correcting for bias
            momentums_int = momentums[k] / (1 - beta1**t)
            velocitys_int = velocitys[k] / (1 - beta2**t)
            second_term = beta1*momentums_int + np.divide((1 - beta1)*grads[k], (1 - beta1**t))
            params[k] -= np.divide(learning_rate, (np.sqrt(velocitys_int + eps))) * second_term
